...
class AddBill extends Component
{
    ...
    public function searchInvestigation()
    {
        ...
        $this->investigationResults = Investigation::where('test_name', 'like', '%' . $search . '%')
            ->orWhere(function ($query) use ($search) {
                $query->where('sell_price', $search); // exact match
            })
            ->limit(30)
            ->get();
    }
    ...
    public function store()
    {
        try {
            ...
            $commissionSettings = Setting::first();
            $commission = json_decode($commissionSettings->commision, true);

            $doctor_commision_type   = $commission['commision_type'] ?? 'percentage';
            $doctor_commision_amount = $commission['commision_amount'] ?? 0;

            if ($this->doctor_id) {
                if ($doctor_commision_type === 'percentage') {
                    $doctor_commision = ($this->total_amount * $doctor_commision_amount) / 100;
                } else {
                    $doctor_commision = $doctor_commision_amount;
                }
                $doctor_commision = intval(round($doctor_commision));
            } else {
                $doctor_commision = 0;
            }
    ...
    public function render()
    {
        ...
        return view('livewire.backend.bills.add-bill', [
            'patients' => $patients,
            'doctors' => $doctors,
            'bill' => $this->bill,
            'qrCode' => $qrCode,
