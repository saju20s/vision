...
class MarketingHistory extends Component
{
    ...
    {
        $this->doctor = Marketing::findOrFail($id);
        $this->loadBills();
    }
    ...
    private function loadBills()
    {
        $query = Bill::with('patient')
            ->where('marketing_id', $this->doctor->id);
        ...
        // $this->bills = $query->orderBy('created_at', 'desc')->get();
    ...
    {
        return view('livewire.backend.marketings.marketing-history')->layout('backend.template.body');
    }
}
